...
const AdminDashboard: React.FC = () => {
  ...
  const fetchTab = useCallback(async (tab: Tab) => {
    ...
    try {
      ...
      if (tab === 'financial') {
        const eR = await apiCall('/admin/doctor-earnings');
        setEarnings(Array.isArray(eR.data) ? eR.data : []);
      }
    } catch (e: any) {
  ...
  useEffect(() => {
    if (['appointments','prescriptions','videocalls','payments','financial'].includes(activeTab)) {
      fetchTab(activeTab as Tab);
    }
  }, [activeTab, fetchTab]);
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                  {(() => {
                    ...
                    const totalConsult  = earnings.reduce((s,e)=>s+(e.stats?.completedConsultations||0),0);
                    const totalRevenu   = totalConsult * 45000;
                    const totalCommiss  = Math.round(totalRevenu * 0.1);
                    const totalVerse    = earnings.reduce((s,e)=>s+(e.stats?.totalPaid||0),0);
                    const resteAVerser  = Math.max(0, Math.round(totalRevenu*0.9) - totalVerse);
                    return [
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
                    ...
                    <div className="space-y-3">
                      {(()=>{
                        const tc = earnings.reduce((s,e)=>s+(e.stats?.completedConsultations||0),0);
                        const tr = tc * 45000;
                        const cm = Math.round(tr*0.1);
        ...
        {activeTab === 'financial' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6">
              ...
              {tabLoading ? <Spinner label="Calcul des revenus…" /> : (() => {
                const totalConsultations = earnings.reduce((s, e) => s + (e.stats?.completedConsultations || 0), 0);
                const totalRevenue       = totalConsultations * 45000;
